...
- **Час — завжди через `Dekanat/utils/clock.py:now_local()`, ніколи `datetime.now()` (DK-50).** `datetime.now()` віддає час у таймзоні процесу; на сервері застосунок працює у контейнері з таймзоною UTC, тому дати створення/редагування карток абітурієнтів і формування звітів зберігались та показувались у UTC замість київського EEST. `now_local()` прив'язаний до явної зони `Europe/Kyiv` (пакет `tzdata` у залежностях гарантує її наявність) і повертає **naive** wall-clock (усі колонки БД — timezone-naive `DateTime`). Використовуйте його скрізь: `default_factory=now_local` у моделях (`created_at`, `generated_at`, `application_status_changed_at`, токени сесії), а також `now_local()`/`now_local().date()` замість `datetime.now()`/`date.today()` у сервісах, DAO і computed-var'ах. Виправлення застосовується **до нових записів** — наявні UTC-мітки в БД не мігруються.
- **Важкі синхронні операції в event-обробнику підвішують застосунок для ВСІХ користувачів (DK-41).** Reflex виконує тіло sync-обробника (`def ...`, включно з кодом між `yield`) прямо в єдиному потоці asyncio (`reflex/state.py`: для sync-генераторів робиться `next(events)` в event loop). Тому важкий рендер документів (`docxtpl`/`lxml`, `openpyxl` — `report.render_bytes()`) блокує обробку websocket-подій усіх клієнтів, доки документ не сформується. Рішення — обробник робимо `async def`, а блокуючу частину (читання даних + рендер) виносимо у фоновий потік через `await run_blocking(fn, ...)` з `Dekanat/utils/background.py` (виділений `ThreadPoolExecutor`). Правила офлоуду: `fn` — `@staticmethod`/чиста функція, приймає примітиви (стан знімаємо на event loop **до** виклику), **не мутує стан і не `yield`-ить**, лише повертає готові `(bytes, filename)`; тости/`rx.download`/`self.downloading` лишаються в async-обробнику. Робота у потоці має бути **read-only** по БД (SQLite `check_same_thread=False` дозволяє крос-потокові сесії, але паралельний INSERT дасть `database is locked` — не виносьте у потік запис). Зразки — `states/rating.py:on_click_download_group/on_click_download_all`, `states/entrants_group.py:on_click_sheet`. **Коли важка операція сама пише в БД (формування рейтингу/звіту, DK-44)** — розділяйте її на дві: `compute_*` (важкий read-only розрахунок, повертає готові дані) виносимо у потік через `run_blocking`, а `persist_*` (швидкий INSERT знімка) лишається на event loop. Так і розрахунок не блокує застосунок, і запис не конфліктує з hot-path авторизації. Зразки — `RatingService.compute_entries/persist_entries` + `states/rating.py:on_click_generate`, `AdmissionCampaignReportService.compute_payload/persist_payload` + `states/admission_campaign_report.py:on_click_generate`.
...
- **Калькулятор комплексного балу (DK-47)** — у діалозі «Результат ЗНО» на формі абітурієнта (`EntrantFormState`/`views/entrant.py:_rz_calc_panel`), не в діалозі іспиту. Кнопка-іконка `calculator` розгортає панель: поле компонентів (через пробіл/кому), перемикач «Середнє»/«Сума» (`rz_calc_mode`, computed `is_rz_calc_avg`), live-результат `rz_calc_result_str`, кнопка «Застосувати» (`rz_calc_apply` кладе результат у поле балу). Балл у діалозі редагується як рядок `rz_points_input` (не reformat'иться при вводі дробу — за зразком `g_grade_input`); парсинг у float — лише на `save_rz`.
...
7. **Скидання при заході (DK-42).** `on_load` завжди очищає `generated_groups`/`_pool`/діалоги — інакше після `on_cancel` (який лише редиректить) повторний вхід показав би старий, уже скасований результат. Видалення цілої чернеткової групи — `remove_group(index)` (перепризначення списку), кнопка `trash_2` у рядку таблиці результатів. Типовий `max_size` при заході береться з глобальної настройки `max_entrants_per_exam_group` (DK-48), користувач може перевизначити на час формування.
8. **Правила бакетів і назв (DK-48, змінює DK-26/DK-42).** Бакет кандидата = (спеціальність найвищого пріоритету, база вступу **особи**, форма навчання цієї спеціальності). **Абітурієнтів без жодної спеціальності пропускаємо.** Назва групи — `<TAG>-<YY><тег>-<N>` (`_bucket_prefix`): `<тег>` = префікс бази вступу, а якщо він порожній — префікс форми навчання; **якщо префікс бази непорожній, префікс форми не використовується взагалі** (навіть якщо заданий). Спільні хелпери `_bucket_prefix` / `_match_existing_groups` / `_next_number` / `_top_specialty` — статичні на сервісі, використовуються і в bulk, і в single-режимі (див. нижче).
9. **Дозаповнення наявних груп (DK-42, оновлено DK-48).** Галочка `use_existing` вмикає режим, коли перед створенням нових груп абітурієнти дозаповнюють уже наявні (не видалені) групи до `max_size`. **Тип наявної групи визначається ВИКЛЮЧНО за її назвою** (`_match_existing_groups` — збіг із префіксом бакета + числовий суфікс): склад (спеціальності/форми/бази учасників) не аналізується взагалі. Групи з довільною назвою, що не відповідає жодному префіксу, не чіпаються. Кількість членів — окремим `EntrantsGroupDao.get_entrant_counts` (щоб урахувати й членів поза діапазоном кампанії); `live_counts` мутуємо при дозаповненні, щоб бакети зі спільним префіксом не переповнили ту саму групу. Топ-ап-групи повертаються з ненульовим `id` та `existing_count`; у `bulk_create_with_entrants` для них викликається `append_entrants` (дописування без чіпання поточного складу), а не `replace_entrants`. Нумерація нових груп (`_next_number`) зважає на назви всіх наявних груп + уже сформованих у цьому запуску, тож індекси не колізять.
10. **Автопідбір групи для одного абітурієнта (DK-48).** `EntrantsGroupService.suggest_group_for_entrant(top_spec_id, base_id, form_id, max_size)` — той самий процес, але для однієї людини й з урахуванням наявних груп. Спершу шукає наявну групу відповідного бакета (за назвою) з вільним місцем (`count < max_size`); якщо нема — пропонує назву нової. Повертає `{"status": "existing"|"new"|"error", "group_id", "title"}`. **У БД нічого не пише** — нову групу створює `EntrantService.add_one/edit_one` через параметр `new_group_title` (`_apply_new_group`, у тій самій транзакції) лише при збереженні картки. Викликається з картки абітурієнта — див. розділ про форму Entrant.
11. **Автопідбір абітурієнтів під конкретну групу (DK-48).** На сторінці **редагування** групи, поруч із кнопкою «+», є кнопка «Автопідбір» (`EditEntrantsGroupState.on_click_autofill`). Вона визначає за назвою групи, під яку спеціальність/базу/форму її створено, і дозаповнює вільними абітурієнтами, що під це підходять, у межах ліміту `max_entrants_per_exam_group`. Визначення «за назвою» — симетричне автоформуванню: `EntrantsGroupService.suggest_entrants_for_group(group_id, exclude_ids)` для кожного вільного кандидата рахує його бакет-префікс (`_bucket_prefix`) і бере тих, чий префікс збігається з назвою групи (`<prefix><N>`) — тобто кого автоформування помістило б у групу з такою назвою (зворотний розбір назви неоднозначний, бо теги бази й форми зливаються в один сегмент). Ліміт застосовує стейт: `free_slots = max_size − len(entrants_in_group)`; додаються лише вільні кандидати, яких ще нема у формі. Абітурієнти йдуть у in-memory склад форми — у БД потрапляють при збереженні (звичайний `edit_one` → `replace_entrants`). Групи зі старим форматом назви (`<TAG>-<YY><база><форма>-<N>`, до DK-48) не збігаються з новими префіксами й нічого не підберуть.
...
### Текст оновлень (changelog) для пользователей (DK-32)

Записи истории обновлений — код, а не UI-редактор: список `UPDATES` в `Dekanat/declared/updates.py`, синхронизация в БД через `AppUpdateService.sync_updates` (по аналогии с `sync_actions` из `update.py`). Каждая запись:

```python
AppUpdate(
    version="0.8",
    title="Короткий заголовок релізу",
    body="- Перший пункт\n- Другий пункт\n- Третій пункт",
)
```

1. **Язык — украинский.** `title`/`body` — пользовательский текст интерфейса, пишется по-украински, как и весь остальной UI.
2. **Формат `body` — без markdown**, рендерится вручную построчно (`ChangelogState._split_lines`, `views/templates/layouts.py:_changelog_row`) — полноценный `rx.markdown` здесь не используется: первое же его применение в проекте упало с `ColorModeContext` при рендере внутри диалога, смонтированного через `extra_app_wraps` (шапка). Правила: строка с `- ` (дефис+пробел) → маркированный пункт списка; любая другая непустая строка → отдельный абзац; пустые строки игнорируются (нельзя использовать для отступов); эмодзи — обычные unicode-символы, отображаются как есть.
3. **`version` — уникальный ключ синхронизации.** Если строка `version` уже есть в БД — запись пропускается целиком, даже если поменять `title`/`body`. Для нового релиза — новая версия (обычно совпадает с `pyproject.toml`); чтобы поправить уже засинхроненную запись, нужна либо правка до первого запуска `update.py`/деплоя, либо новая `version`.
4. **Порядок** — просто дописывать новую запись в конец `UPDATES`; в диалоге сортировка идёт по `published_at desc, id desc`, так что она окажется сверху.
5. **Применение** — `uv run python update.py` локально (на проде это же входит в `deploy.py`). Только после синка новая запись появляется в БД, у пользователей, которые её не видели, загорается маркер на колокольчике и при следующем `require_auth` показывается тост «Систему оновлено! 🎉» (см. `AppState._check_unread_update`, `WorkerModel.last_seen_update_id` — high-watermark, без отдельной M2M-таблицы «просмотрено»).
...
| Item ZNO | `ItemZnoModel` | `/base/item_zno` | Предмети ЗНО. Поле `coefficient` (float, дефолт 1.0) — ваговий коефіцієнт: бал, введений оператором/на тестуванні, домножається на нього при збереженні оцінки (DK-40). Поле `is_counted_in_rating` (bool, дефолт False, DK-47) — чи входить оцінка предмета у суму балів рейтингу та у колонки DOCX рейтингу; предмети з `False` (напр. окремі компоненти НМТ) ігноруються при формуванні рейтингу. Бекфіл існуючих = False — після міграції предмети треба явно вмикати. |
| Entrants group | `EntrantGroupModel` | `/admission_commission/entrants_group` | Групи абітурієнтів на ЗНО. Список показує кількість абітурієнтів у групі та сортується за назвою й кількістю (DK-42/DK-48). Назва автоформованих груп — `<TAG>-<YY><тег>-<N>`, де `<тег>` = префікс бази вступу або, якщо він порожній, префікс форми навчання (DK-48). Окремо: `/auto` — автоформування за спеціальністю найвищого пріоритету активної кампанії з рівномірним розподілом (DK-24); абітурієнти без спеціальностей пропускаються (DK-48); опція дозаповнення наявних груп (їх тип визначається лише за назвою — DK-48) і видалення зайвих чернеткових груп (DK-42); `/print?ids=…` — друк складу обраних груп. Право `entrants_group:auto_generate` гейтить кнопку, доступ на сторінку та сам запуск формування. Автопідбір групи для одного абітурієнта — прямо в його картці (`suggest_group_for_entrant`, DK-48). На сторінці редагування групи — кнопка «Автопідбір» (`suggest_entrants_for_group`, DK-48): дозаповнює групу вільними абітурієнтами, що пасують її назві, у межах ліміту. |
...
| App settings | `AppSettingModel` | `/admin/settings` | Загальні налаштування системи (key/value/category/value_type). Одна сторінка, секції за `category`: «Авторизація» (`session_timeout_minutes`), «Рейтинг» (`max_total_points` — верхня межа суми балів при формуванні рейтингу, дефолт 200, DK-40) та «Групи ЗНО» (`max_entrants_per_exam_group` — макс. абітурієнтів у екз. групі, дефолт 25, DK-48: типове значення при автоформуванні, ліміт при автопідборі групи в картці та поріг попередження при ручному виборі). Дефолтні записи сидяться у `deploy.py` через `AppSettingService.ensure_defaults()`. Права: `settings:view` / `settings:edit`. |
...
| Журнал реєстрації заяв | (окремої моделі немає — читає `EntrantModel`/`PersonModel`) | `/reporting/registration_journal` | Журнал реєстрації заяв вступників (офіційна форма МОН, DK-30): хронологічний перелік поданих заяв (за датою прийому `created_at` asc) за вибраною кампанією з фільтром дати «день/період» (дефолт — весь період активної кампанії). **Живий перегляд без знімка** — пряма проекція даних абітурієнтів (власного DAO немає, переиспользує `EntrantDao.get_all`); плюс вивантаження DOCX за шаблоном `reports/templates/registration_journal.docx` (важкий рендер — у фоновий потік через `run_blocking`, read-only). Один рядок = один абітурієнт, 10 колонок форми. Заповнюються з БД: 1 ЄДЕБО, 2 дата прийому, 3 ПІБ, 4 стать (Чол./Жін.), 5 дата народження, 6 документ про освіту, 8 результати ЗНО (бали `results_zno.points` — саме ті, що беруть участь у рейтингу). Колонки 7 (пріоритетність), 9 (причини відмови), 10 (підпис про повернення) лишаються порожніми (заповнюються від руки). Тільки `view`/`docx` — без CRUD. Права: `report_journal:view` / `report_journal:docx`. |
...
- На формі **Entrant** (`views/entrant.py`, `states/entrant.py:EntrantFormState`):
  ...
  **Вибір екзаменаційної групи (DK-48)** живе на вкладці «Спеціальності», під таблицею специальностей (`_f_group_block`), а не в персональній інформації. Ручний вибір зі списку доступний завжди; кнопка «Визначити автоматично» зʼявляється, коли додано ≥1 спеціальність, і викликає `on_click_autodetect_group` → `EntrantsGroupService.suggest_group_for_entrant`. Якщо підбір запропонував **нову** групу — її назва тримається в `pending_new_group_title` (id_entrant_group=0) і потрапляє в БД лише при збереженні картки (`new_group_title`); ручний вибір групи скидає `pending_new_group_title`. Computed `group_limit_warning` показує попередження під полем, якщо в обраній наявній групі вже досягнуто ліміту (`max_entrants_per_exam_group`); самого абітурієнта, якщо він уже в цій групі, за «зайвого» не рахуємо (`loaded_entrant_group_id`).
...
8. **Рейтинг**: `rating_snapshots (id_campaign, generated_at) ← rating_entries (id_speciality, id_entrant, position, total_points, status)`. Знімок формується на вимогу через `RatingService.generate(id_campaign)`: сума `results_zno.points` **лише по предметах з `item_zno.is_counted_in_rating = True`** (решта — напр. компоненти НМТ — не входять у суму, DK-47) по особі (обрізана зверху до `max_total_points` з налаштувань, дефолт 200, DK-40) + ознака «квота» (`special_conditions_person → special_conditions.is_kvota = True`) → впорядкування «квоти зверху, далі за сумою балів desc» → розподіл по квотах `admission_campaigns_specialties` у статуси `kvota`/`budget`/`contract`/`rejected`. Абітурієнти, чий `application_status.is_allowed_in_rating = False` (дефолт), у ранжуванні не беруть участі: місць не займають, отримують статус `excluded` і найбільші `position` (самий низ таблиці, сірий рядок; DK-43). До офіційного DOCX-документа вони не потрапляють; у DOCX колонки оцінок — теж лише по предметах `is_counted_in_rating` (DK-47). Попередній знімок кампанії перезаписується (історія не зберігається). **`results_zno.points`/`points_raw` та `rating_entries.total_points` — дробні (float, DK-47)**: комплексні бали (напр. середнє за НМТ) не цілі. **`results_zno.points` зберігає вже домножений на коефіцієнт предмета бал** (множення відбувається при збереженні оцінки — у діалозі картки абітурієнта та у діалозі оцінювання іспиту); сирий введений бал лежить у `results_zno.points_raw` (щоб повторне редагування не домножувало вдруге). Дробні оцінки для відображення форматуються через `Dekanat/utils/display.py:format_grade` (ціле — без «.0»).
