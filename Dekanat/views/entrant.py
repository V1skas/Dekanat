...
def _list_table_row(item: EntrantModel) -> rx.Component:
    return rx.table.row(
        rx.cond(
            ListEntrantState.select_mode,
            rx.table.cell(
                rx.checkbox(
                    checked=ListEntrantState.selected_set.contains(item.id.to_string()),
                    on_change=ListEntrantState.toggle_selected(item.id),
                ),
                width="2.5rem",
            ),
        ),
...
def _list_header_actions() -> rx.Component:
    """Кнопки шапки списку. У режимі вибору (DK-68) — тулбар масової зміни статусу
    замість звичайних кнопок."""
    return rx.cond(
        ListEntrantState.select_mode,
        rx.hstack(
            controls.button_secondary("Виділити всі", on_click=ListEntrantState.select_all),
            controls.button_secondary("Зняти виділення", on_click=ListEntrantState.clear_selection),
            controls.button_primary("Змінити статус", on_click=ListEntrantState.open_bulk_status_dialog),
            controls.button_image_primary(name_icon="circle_x", on_click=ListEntrantState.toggle_select_mode),
            spacing="2",
        ),
        rx.hstack(
            controls.button_filter_toggle(ListEntrantState.filter_open, on_click=ListEntrantState.toggle_filter),
            # Масова зміна статусу — лише у загальному представленні й за наявності
            # права на зміну статусу (DK-68).
            rx.cond(
                (ListEntrantState.current_view == VIEW_GENERAL)
                & ListEntrantState.get_user_actions.contains(Actions.ENTRANT_EDIT_STATUS),
                controls.button_image_secondary(
                    name_icon="list-checks", on_click=ListEntrantState.toggle_select_mode,
                ),
            ),
            rx.cond(
...
def _bulk_status_dialog() -> rx.Component:
    """Діалог вибору цільового статусу для масової зміни (DK-68)."""
    return rx.dialog.root(
        rx.dialog.content(
            rx.dialog.title("Масова зміна статусу"),
            rx.dialog.description(
                "Обрано абітурієнтів: "
                + ListEntrantState.selected_count.to_string()
                + ". Оберіть новий статус заяви — його буде застосовано до всіх обраних.",
            ),
            rx.vstack(
                _select(
                    ListEntrantState.application_status_options,
                    ListEntrantState.bulk_status_id_str,
                    ListEntrantState.set_bulk_status_id,
                    placeholder="Оберіть статус",
                    width="100%",
                ),
                rx.hstack(
                    controls.button_secondary("Скасувати", on_click=ListEntrantState.set_bulk_status_open(False)),
                    controls.button_primary("Застосувати", on_click=ListEntrantState.apply_bulk_status),
                    spacing="2",
                    justify="end",
                    width="100%",
                ),
        ...
        open=ListEntrantState.bulk_status_open,
        on_open_change=ListEntrantState.set_bulk_status_open,
    )
