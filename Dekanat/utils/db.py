...
def register_ua_collation() -> None:
    ...
    def _setup_collation(dbapi_connection, connection_record):  # noqa: ARG001
        ...
        # Вбудований SQLite lower() не знає кирилицю ("Захарченко" лишається як є),
        # тому пошук із Python .lower() по той бік нічого не знаходить. Реєструємо
        # власну ua_lower на базі Python str.lower(), що коректно опускає кирилицю.
        dbapi_connection.create_function("ua_lower", 1, _py_lower, deterministic=True)

    _registered = True


def _py_lower(value: Optional[str]) -> Optional[str]:
...
def ua_lower(col):
    """Регістронезалежне приведення тексту з підтримкою кирилиці — для LIKE-пошуку.

    На SQLite вбудований `lower()` не опускає кириличні літери, тому пошук, де
    запит приведено до нижнього регістру в Python, не знаходив збігів (DK-36).
    Використовуємо власну `ua_lower`, зареєстровану на connect. На MySQL/інших —
    нативний `lower()` (він уже Unicode-aware).
    """
    try:
        dialect = get_engine().dialect.name
    except Exception:
