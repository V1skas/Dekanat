...
class ListDepartmentState(AppState):
    items: Optional[Sequence[DepartmentModel]] = None
    in_progress: bool = True

    @rx.event
    def on_load(self):
        if not self.has_permission(Actions.DEPARTMENT_LIST):
    ...
    def on_click_add(self):
        return rx.redirect(routes.DEPARTMENT_ADD)


class AddDepartmentState(AppState):
    item: DepartmentModel = DepartmentModel()
    in_process: bool = False

    def _reload_item(self):
        self.item = DepartmentModel()

    @rx.event
    def on_load(self):
        if not self.has_permission(Actions.DEPARTMENT_ADD):
    ...
    @rx.event
    def on_cancel(self):
        return rx.redirect(routes.DEPARTMENT_LIST)


class EditDepartmentState(AppState):
    item: DepartmentModel = DepartmentModel()
    ...
    @rx.event
    def on_load(self):
        if not self.has_permission(Actions.DEPARTMENT_EDIT):
            yield rx.toast.error("У Вас немає доступу до цієї сторінки!")
            yield rx.redirect(routes.DASHBOARD)
            return
    ...
    @rx.event
    def on_cancel(self):
        return rx.redirect(routes.DEPARTMENT_VIEW + str(self.router.page.params.get("id", "")))


class ViewDepartmentState(AppState):
    item: DepartmentModel = DepartmentModel()
    in_process: bool = True
    ...
    @rx.event
    def on_load(self):
        if not self.has_permission(Actions.DEPARTMENT_VIEW):
            yield rx.toast.error("У Вас немає доступу до цієї сторінки!")
            yield rx.redirect(routes.DASHBOARD)
            return
    ...
    def on_click_edit(self):
        ...
        return rx.redirect(routes.DEPARTMENT_EDIT + str(self.item.id))

    @rx.event
    def on_click_delete(self):
        if not self.has_permission(Actions.DEPARTMENT_DELETE):
            yield rx.toast.error("У Вас немає дозволу на виконання цієї дії!")
            return
